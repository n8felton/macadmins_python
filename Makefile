...
# Minimum supported macOS version for Python 3.11+ is 11.0, so we can hardcode this value
MACOS_VERSION := 11
...
PYTHON_BIN := $(MANAGEDFRAMEWORKS_PYTHON_PATH)/Python3.framework/Versions/Current/Resources/Python.app/Contents/MacOS/Python
...
clean_brew:
ifdef CI
	@brew remove $$(brew list) 2>/dev/null || true
	@brew install jq
endif
